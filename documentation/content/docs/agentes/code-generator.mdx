...
- **Fallback Playwright (se WebSearch/WebFetch falhar para entry points):**
  ...
  Se `WebSearch` ou `WebFetch` retornarem erro, timeout ou conteudo vazio/ilegivel durante a busca TDN para entry points, utilize as ferramentas Playwright MCP como fallback:
  ...
  - Rotina padrao que aciona o entry point
